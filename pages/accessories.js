...
const Accessories = ({ products }) => {
  return (
    <div>
       <Head>
        <title>ACCESSORIES - XenithGoods</title>
      ...
      <section className="min-h-screen text-gray-600 body-font">
        <div className="container px-5 md:py-24 py-12 mx-auto">
          <div className="flex justify-center flex-wrap -m-4  mx-5 text-center">
            {Object.keys(products).length === 0 && <p className='text-gray-50 font-mono'>Sorry all the Accessories products are currently out of stock. New stock coming soon. Stay Tuned!</p>}
                ...
                <div key={products[item]._id} className="lg:w-1/5 md:w-1/2 p-4 w-full cursor-pointer shadow-xl m-5">
                  <Link passHref={true} href={`/product/${products[item].slug}`}>
                    <div className="block relative rounded overflow-hidden">
                      <img alt="ecommerce" className="m-auto h-[32vh] block" src={products[item].img} />
                    </div>
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: 'accessorie' })
