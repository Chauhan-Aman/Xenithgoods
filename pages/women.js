...
const Women = ({ products }) => {
    ...
    const [price, setPrice] = useState([0,12500])
    ...
    return (
        <div>
            ...
            <div className='flex'>
                    ...
                    <div className='flex flex-col'>
                        ...
                        <div className='my-2 mb-6'>
                            ...
                            >
                                ...
                                <Checkbox value="Dress" size='sm' color='default'><p className='text-gray-300 w-20'>Dress</p></Checkbox>
                                <Checkbox value="Cardigan" size='sm' color='default'><p className='text-gray-300 w-20'>Cardigan</p></Checkbox>
                                <Checkbox value="Suit" size='sm' color='default'><p className='text-gray-300 w-20'>Suit</p></Checkbox>
                                <Checkbox value="Saree" size='sm' color='default'><p className='text-gray-300 w-20'>Saree</p></Checkbox>
                            </CheckboxGroup>
                        </div>
                ...
                <Card type={'Women'} products={filteredProducts} CheckSize={sizebox} CheckColor={colorbox} CheckType={typebox} price={price} />
...
export async function getServerSideProps(context) {
    ...
    for (let item of products) {
        if (item.title in womens) {
            if (!womens[item.title].color.includes(item.color) && item.availableQty > 0) {
                womens[item.title].color.push(item.color)
            }
            if (!womens[item.title].size.includes(item.size) && item.availableQty > 0) {
                womens[item.title].size.push(item.size)
            }
        }
        else {
            womens[item.title] = JSON.parse(JSON.stringify(item))
            if (item.availableQty > 0) {
                womens[item.title].color = [item.color]
                womens[item.title].size = [item.size]
            }
            else {
                womens[item.title].color = []
                womens[item.title].size = []
            }
        }
    }
    return {
        props: { products: JSON.parse(JSON.stringify(womens)) },
    }
}
